#!/usr/bin/env python3
"""Q4: ranking, beyond-accuracy, slice, and bootstrap evaluation harness."""
...
def run_method(name, rows, score_fn, vectors, positions, popularity, catalog_size):
    metric_values = {key: [] for key in ["auc", "mrr", "ndcg@5", "ndcg@10", "diversity@10", "novelty@10"]}; recommendations = []; slice_values = {"cold_history_under_5": {key: [] for key in metric_values}, "warm_history_5_or_more": {key: [] for key in metric_values}}
    ...
    for row in rows.itertuples(index=False):
        ...
        bucket = "cold_history_under_5" if len(row.history_ids) < 5 else "warm_history_5_or_more"
        for key, value in values.items(): metric_values[key].append(value); slice_values[bucket][key].append(value)
        recommendations.append(top)
    # Coverage is global: proportion of the catalog recommended at least once.
...
def evaluate_dataset(store: Path, dataset: str, limit: int):
    ...
    return {"dataset": dataset, "catalog_articles": len(ids), "methods": [run_method("bm25", rows, lexical, vectors, positions, popularity, len(ids)), run_method("semantic", rows, semantic, vectors, positions, popularity, len(ids))]}

def main():
    p = argparse.ArgumentParser(); p.add_argument("--store", type=Path, default=Path("data/processed")); p.add_argument("--limit", type=int, default=0); p.add_argument("--output", type=Path, default=Path("outputs/q4_evaluation.json")); a = p.parse_args()
