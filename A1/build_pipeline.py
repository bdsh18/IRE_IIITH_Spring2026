#!/usr/bin/env python3
"""Q1 one-command, reproducible raw-data to feature-store pipeline.

Only MIND-small and EB-NeRD demo are extracted/processed.  Large archives are
never touched by this development command.
"""
...
def extract_once(archive: Path, target: Path) -> None:
    """Extract a raw archive only when its expected folder/files are absent."""
...
def recency_weights(history: list[str], decay: float = .8) -> list[float]:
    """Newest click receives weight 1; older clicks decay exponentially."""
...
def load_mind(folder: Path, split: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return articles[["dataset", "source_split", "article_id", "title", "abstract", "body", "category", "entities", "embedding_available", "embedding_path"]], impressions
