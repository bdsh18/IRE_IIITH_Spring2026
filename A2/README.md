...
This is the A2 repository. It builds on Assignment 1 retrieval components, then trains an in-view behavioural re-ranker for MIND and EB-NeRD, with a separately measured catalog-retrieval stage.

## One-command reproduction

Create and activate a virtual environment, then run the full pipeline:
...
```bash
./run_a2_pipeline.sh --smoke-limit 500
```
...
```bash
source .venv/bin/activate
SOURCE_ROOT=../../Assighnment1
```

To generate both ZIPs sequentially using the auto-detected A1 download folder, use:
...
```bash
source .venv/bin/activate
python mind_large_v2.py train --sample-modulus 10 --negatives-per-positive 20
python mind_large_v2.py evaluate --model data/v2/mind_large/mind_v2.pkl
# Inspect outputs/mind_v2_validation.json before continuing.
python mind_large_v2.py train --include-dev-in-train --sample-modulus 10 --negatives-per-positive 20 \
  --model data/v2/mind_large/mind_v2_final_mod10.pkl
python mind_large_v2.py submit --model data/v2/mind_large/mind_v2_final_mod10.pkl \
...
```bash
python ebnerd_large_v2.py --mode all
# Inspect data/v2/ebnerd_large/validation_metrics.json before continuing.
python ebnerd_large_v2.py --mode submit --prediction-batch-size 6000
```
